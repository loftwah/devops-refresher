...
* Idempotent file naming such as s3://bucket/raw/source=foo/ingest\_date=YYYY=MM=DD/run\_id=UUID.
...
* Store operational metadata such as run\_id, source offsets and counts.
...
* Ingestion: AWS DMS or custom extractors on ECS to S3 raw zone as Parquet partitioned by ingest\_date.
...
5. **Ingest raw**
   ...
   * Drop Parquet into s3://org-data-raw-ap-southeast-2/source=app/ingest\_date=YYYY=MM=DD/.
...
7. **Quality checks**
   ...
   * Add tests for not\_null, unique and accepted\_values.
