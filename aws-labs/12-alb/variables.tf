...
variable "public_subnet_ids"   { description = "Public subnet IDs (optional; read from Lab 01 if empty)" type = list(string) default = [] }
variable "alb_sg_id"           { description = "ALB security group ID" type = string }
variable "target_port"         { description = "Target port" type = number default = 3000 }
variable "health_check_path"   { description = "Health check path" type = string default = "/healthz" }
